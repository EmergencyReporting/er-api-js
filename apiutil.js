...
const missingRequiredFields = (obj, requiredFieldNames = []) =>
  requiredFieldNames.reduce((missingReqs, req) => {
...
module.exports = {
  erApiPostWithParams: (uriPart, requiredFields = []) => data =>
    verifyRequired(data, requiredFields).then(data =>
  ...
  erApiGetV1WithParams: uriPart => p =>
    getAccessToken().then(accessToken => {
      let params = p;
      let headers = {
        Authorization: accessToken
  ...
  erApiGetV2WithParams: uriPart => params =>
    getAccessToken().then(accessToken =>
      axios({
        url: `${getApiBaseUrl()}${uriPart}`,
        method: "get",
